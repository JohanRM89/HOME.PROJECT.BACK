  ...
  static register() {
    eventBus.on(EVENTS.TASK_ASSIGNED, async ({ task, assignedUser }) => {
      try {
        ...
        console.error("[NotificationObserver] TASK_ASSIGNED:", e.message);
      }
    });
    ...
    eventBus.on(EVENTS.TASK_STATUS_CHANGED, async ({ task, actor }) => {
      try {
        console.log(
          `[NotificationObserver] STATUS_CHANGED: Notificando a ${task} sobre cambio de estado de "${task.title}"`,
        );
        await NotificationRepository.create({
          user_id: actor.id,
          task_id: task.id,
        ...
        console.error("[NotificationObserver] STATUS_CHANGED:", e.message);
      }
    });
    ...
    eventBus.on(EVENTS.TASK_DUE_SOON, async ({ tasks }) => {
      try {
        ...
        console.error("[NotificationObserver] DUE_SOON:", e.message);
      }
    });
    ...
    eventBus.on(EVENTS.REPORT_GENERATED, async ({ report, groupMembers }) => {
      try {
        ...
        console.error("[NotificationObserver] REPORT:", e.message);
      }
    });
    ...
    eventBus.on(
      EVENTS.USER_JOINED_GROUP,
      async ({ actor, famiId, groupMembers }) => {
        try {
          // const notifications = groupMembers
          //   .filter((uid) => uid !== actor.id)
          //   .map((uid) => ({
          ...
          //     type: "family",
          //     group_id: famiId,
          //     message: `${actor.name} se unió al grupo`,
          //   }));
