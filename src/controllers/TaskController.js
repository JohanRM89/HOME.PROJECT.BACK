...
class TaskController {
  ...
  async getCalendar(req, res, next) {
    try {
      ...
      if (!dia) {
        return ResponseView.badRequest(res, "El día es obligatorio");
      }

      const tasks = await TaskService.getCalendar(userId, groupId, dia);

      return ResponseView.success(res, tasks);
    } catch (error) {
      next(error);
    }
  }
